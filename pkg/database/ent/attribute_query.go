...
type AttributeQuery struct {
	...
	withAttributeType *AttributeTypeQuery
...
func (aq *AttributeQuery) QueryCertification() *CertificationQuery {
...
// QueryAttributeType chains the current query on the "attributeType" edge.
func (aq *AttributeQuery) QueryAttributeType() *AttributeTypeQuery {
	query := (&AttributeTypeClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AttributeQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (aq *AttributeQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (aq *AttributeQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (aq *AttributeQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (aq *AttributeQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (aq *AttributeQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := aq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (aq *AttributeQuery) Clone() *AttributeQuery {
	...
	return &AttributeQuery{
		...
		withAttributeType: aq.withAttributeType.Clone(),
...
func (aq *AttributeQuery) WithCertification(opts ...func(*CertificationQuery)) *AttributeQuery {
	query := (&CertificationClient{config: aq.config}).Query()
...
// the "attributeType" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AttributeQuery) WithAttributeType(opts ...func(*AttributeTypeQuery)) *AttributeQuery {
	query := (&AttributeTypeClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withAttributeType = query
	return aq
}
...
func (aq *AttributeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Attribute, error) {
	...
	if query := aq.withCertification; query != nil {
		if err := aq.loadCertification(ctx, query, nodes, nil,
			func(n *Attribute, e *Certification) { n.Edges.Certification = e }); err != nil {
			return nil, err
		}
	}
	if query := aq.withAttributeType; query != nil {
		if err := aq.loadAttributeType(ctx, query, nodes, nil,
			func(n *Attribute, e *AttributeType) { n.Edges.AttributeType = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AttributeQuery) loadCertification(ctx context.Context, query *CertificationQuery, nodes []*Attribute, init func(*Attribute), assign func(*Attribute, *Certification)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Attribute)
	for i := range nodes {
		fk := nodes[i].CertUUID
	...
	query.Where(certification.IDIn(ids...))
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "certUUID" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (aq *AttributeQuery) loadAttributeType(ctx context.Context, query *AttributeTypeQuery, nodes []*Attribute, init func(*Attribute), assign func(*Attribute, *AttributeType)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Attribute)
	for i := range nodes {
...
func (aq *AttributeQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(attribute.Table, attribute.Columns, sqlgraph.NewFieldSpec(attribute.FieldID, field.TypeUUID))
	_spec.From = aq.sql
...
func (aq *AttributeQuery) ForUpdate(opts ...sql.LockOption) *AttributeQuery {
...
func (aq *AttributeQuery) ForShare(opts ...sql.LockOption) *AttributeQuery {
	if aq.driver.Dialect() == dialect.Postgres {
		aq.Unique(false)
	}
	aq.modifiers = append(aq.modifiers, func(s *sql.Selector) {
