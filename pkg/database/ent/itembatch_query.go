...
func (ibq *ItemBatchQuery) QueryCompany() *CompanyQuery {
	query := &CompanyQuery{config: ibq.config}
...
func (ibq *ItemBatchQuery) IDs(ctx context.Context) ([]uuid.UUID, error) {
	var ids []uuid.UUID
...
func (ibq *ItemBatchQuery) GroupBy(field string, fields ...string) *ItemBatchGroupBy {
	grbuild := &ItemBatchGroupBy{config: ibq.config}
...
func (ibq *ItemBatchQuery) Select(fields ...string) *ItemBatchSelect {
	...
	selbuild := &ItemBatchSelect{ItemBatchQuery: ibq}
	selbuild.label = itembatch.Label
...
func (ibq *ItemBatchQuery) sqlExist(ctx context.Context) (bool, error) {
...
func (ibgb *ItemBatchGroupBy) sqlScan(ctx context.Context, v any) error {
...
func (ibgb *ItemBatchGroupBy) sqlQuery() *sql.Selector {
...
func (ibs *ItemBatchSelect) Scan(ctx context.Context, v any) error {
	...
	ibs.sql = ibs.ItemBatchQuery.sqlQuery(ctx)
	return ibs.sqlScan(ctx, v)
}

func (ibs *ItemBatchSelect) sqlScan(ctx context.Context, v any) error {
	aggregation := make([]string, 0, len(ibs.fns))
	for _, fn := range ibs.fns {
		aggregation = append(aggregation, fn(ibs.sql))
	}
	switch n := len(*ibs.selector.flds); {
	case n == 0 && len(aggregation) > 0:
		ibs.sql.Select(aggregation...)
	case n != 0 && len(aggregation) > 0:
		ibs.sql.AppendSelect(aggregation...)
	}
	rows := &sql.Rows{}
